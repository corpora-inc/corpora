...
// stub these out in /ts/commander/src/components/
...
export default function ProjectEditorPage() {
    ...
    const projectQuery = useCorporaCommanderApiProjectGetProject(id!, { query: { enabled: !!id } })
    const sectionsQuery = useCorporaCommanderApiSectionListSections(id!, { query: { enabled: !!id } })
    ...
    // hydrate store
    useEffect(() => { if (projectQuery.data) setProject(projectQuery.data.data) }, [projectQuery.data, setProject])
    useEffect(() => { if (sectionsQuery.data) setSections(sectionsQuery.data.data) }, [sectionsQuery.data, setSections])

    if (!id) return <p className="p-4 text-red-600">No project ID provided.</p>
    if (projectQuery.isLoading || sectionsQuery.isLoading) {
        return <div className="flex h-full items-center justify-center">
            <Loader2 className="animate-spin h-6 w-6 text-gray-500" />
        </div>
    }
    if (projectQuery.isError) return <p className="p-4 text-red-600">Error loading project: {projectQuery.error?.message}</p>
    if (sectionsQuery.isError) return <p className="p-4 text-red-600">Error loading sections: {sectionsQuery.error?.message}</p>

    // clear lower-level selection when changing level
    ...
    return (
        <>
                ...
                <aside className="w-64 border-r p-4 hidden md:block">
                    ...
                    {sections.length === 0 ? (
                        <div className="space-y-2">
                            ...
                            <Button onClick={() => setIsOutlineOpen(true)}>Generate outline</Button>
                        </div>
                    ) : (
                ...
                <main className="flex-1 overflow-y-auto p-6">
                    <header className="mb-6 border-b pb-4 flex items-center justify-between">
                        ...
                        <div className="space-x-2">
                            {sections.length > 0 && <Button onClick={() => setIsDraftOpen(true)}>Draft book</Button>}
            ...
            <GenerateOutlineDialog open={isOutlineOpen} onClose={() => setIsOutlineOpen(false)} />
            <GenerateDraftDialog open={isDraftOpen} onClose={() => setIsDraftOpen(false)} />
        </>
    )
}
